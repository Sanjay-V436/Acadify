...
# Remove faculty records without a valid name
...
# Create searchable faculty profile
faculty["text"] = (
...
def get_relevant_names(row):
    """
    Ground truth comes from relevant_faculty_names column.
    Example:
    Dr N Aishwarya | Dr C Ganesh Kumar
    """

    if pd.isna(row["relevant_faculty_names"]):
        return set()

    names = str(row["relevant_faculty_names"]).split("|")
...
def hit_at_k(recommended, relevant, k):
    ...
    return int(
        any(name in relevant for name in top_k)
    )
...
for _, student in students.iterrows():
    ...
    hit1 = hit_at_k(recommended_names, relevant, 1)
    hit3 = hit_at_k(recommended_names, relevant, 3)
    hit5 = hit_at_k(recommended_names, relevant, 5)

    recall1 = recall_at_k(recommended_names, relevant, 1)
    recall3 = recall_at_k(recommended_names, relevant, 3)
    recall5 = recall_at_k(recommended_names, relevant, 5)
...
results_df.to_csv(
    "results/tfidf_student_results.csv",
    index=False
)

summary_df.to_csv(
    "results/tfidf_summary.csv",
    index=False
)
...
print("results/tfidf_student_results.csv")
print("results/tfidf_summary.csv")